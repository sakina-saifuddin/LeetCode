class Solution:
    def orangesRotting(self, grid: List[List[int]]) -> int:
        ...
        time, fresh=0,0
        q=deque() #store rotten oranges in queue
        ...
        while q and fresh>0:
            ...
            for _ in range(len(q)):
                ...
                for dr, dc in direction:
                    fresh_r, fresh_c=(dr+rotten_r), (dc+rotten_c)

                    if fresh_r>=0 and fresh_c>=0 and fresh_r in range(totalrows) and fresh_c in range(totalcols) and grid[fresh_r][fresh_c]==1:
